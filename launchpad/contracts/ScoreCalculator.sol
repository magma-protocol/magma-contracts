...
import "@magmaswap/periphery/contracts/libraries/PositionValue.sol";
import "@magmaswap/periphery/contracts/libraries/PoolAddress.sol";
...
contract ScoreCalculator is IScoreCalculator, Initializable {
   ...
   // If the actual price is 0.2, then price = 0.2 * 10^8 = 20000000
    ...
    function setMagmaToken(address magmaToken_) external onlyOwner {
        require(magmaToken_ != address(0), "zero address");
        magmaToken = magmaToken_;
    }
    ...
    function calculate(address token, uint256 tokenIdOrAmount) external view override returns (uint256 score) {
        ...
        if (token == magmaToken) score = _calculateMagmaToken(tokenIdOrAmount);
    ...
    function _calculatePosition(uint256 tokenId) internal view returns (uint256 score) {
        ...
        uint256 ratio;
        {
            (sqrtPriceX96, , , , , , ) = IMagmaPool(pool).slot0();
    ...
    function _calculateMagmaToken(uint256 amount) internal pure returns (uint256 score) {
    //    MagmaToken has a precision of 18, while "score" has a decimals of 8. Therefore, it needs to be divided by 10^(18-8).
        score = amount / 10**10;
    }
