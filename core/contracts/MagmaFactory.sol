...
/// @title Canonical Magma factory
/// @notice Deploys Magma pools and manages ownership and control over pool protocol fees
contract MagmaFactory is IMagmaFactory {
    /// @inheritdoc IMagmaFactory
    ...
    function createPool(
        ...
        pool = IMagmaPoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IMagmaPool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IMagmaPool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IMagmaPool(pool).setLmPool(lmPool);
    }
}
