...
contract MagmaPoolDeployer is IMagmaPoolDeployer {
    struct Parameters {
        address factory;
        address token0;
        address token1;
    ...
    /// @param factory The contract address of the Magma factory
    ...
    function deploy(
        ...
        pool = address(new MagmaPool{salt: keccak256(abi.encode(token0, token1, fee))}());
        delete parameters;
    }
}
