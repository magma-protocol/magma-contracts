 ...
 * Receiver will harvest mama from MasterChefV2 pool , then upkeep for MasterChefV3.
 */
contract MasterChefV3KeeperV1 is KeeperCompatibleInterface, Ownable, Pausable {
    ...
    /// @param _mama Mama address.
    /// @param _V2Pid Pid in MasterChefV2.
    constructor(IMasterChefV2 _V2, IMasterChefV3 _V3, IReceiver _receiver, IERC20 _mama, uint256 _V2Pid) {
        MasterChefV2 = _V2;
        MasterChefV3 = _V3;
        Receiver = _receiver;
        Mama = _mama;
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory) {
        if (!paused()) {
            ...
            uint256 mamaBalanceInReceiver = Mama.balanceOf(address(Receiver));
            uint256 latestPeriodEndTime = MasterChefV3.latestPeriodEndTime();
            if (penddingMamaAmount + mamaBalanceInReceiver > 0 && latestPeriodEndTime < block.timestamp + bufferSecond)
                upkeepNeeded = true;
        }
    }
