...
contract MasterChef is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice Record the mama amount belong to MasterChefV3.
    uint256 public mamaAmountBelongToMC;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IMagmaPool indexed pool, ILMPool indexed lmPool);
    ...
    /// @return mamaPerSecond Mama reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256 mamaPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            mamaPerSecond = (latestPeriodMamaPerSecond * poolInfo[_pid].allocPoint) / totalAllocPoint;
        }
        endTime = latestPeriodEndTime;
    }

    /// @notice Returns the mama per second , period end time. This is for liquidity mining pool.
    /// @param _pool Address of the V3 pool.
    /// @return mamaPerSecond Mama reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _pool) public view returns (uint256 mamaPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            mamaPerSecond =
    ...
    /// @notice View function for checking pending MAMA rewards.
    /// @dev The pending mama amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        if (MAMA.allowance(_receiver, address(this)) != type(uint256).max) revert();
        receiver = _receiver;
    ...
    function add(uint256 _allocPoint, IMagmaPool _pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's MAMA allocation point. Can only be called by the owner.
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotMagmaNFT();
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce mamaAmountBelongToMC.
        if (_token == address(MAMA)) {
            unchecked {
                // In fact balance should always be greater than or equal to mamaAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balance >= mamaAmountBelongToMC) {
                    balance -= mamaAmountBelongToMC;
                } else {
                    // This should never happend.
                    mamaAmountBelongToMC = balance;
                    balance = 0;
                }
            }
        }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        // Need to reduce mamaAmountBelongToMC.
        if (token == address(MAMA)) {
            unchecked {
                // In fact balance should always be greater than or equal to mamaAmountBelongToMC, but in order to avoid any unknown issue, we added this check.
                if (balanceToken >= mamaAmountBelongToMC) {
                    balanceToken -= mamaAmountBelongToMC;
                } else {
                    // This should never happend.
                    mamaAmountBelongToMC = balanceToken;
                    balanceToken = 0;
                }
            }
        }
    ...
    /// @param _amount The amount of mama injected.
    ...
    function upkeep(uint256 _amount, uint256 _duration, bool _withUpdate) external onlyReceiver {
        // Transfer mama token from receiver.
        MAMA.safeTransferFrom(receiver, address(this), _amount);
        // Update mamaAmountBelongToMC
        unchecked {
            mamaAmountBelongToMC += _amount;
        }
        ...
        if (latestPeriodEndTime > currentTime) {
            ...
            mamaAmount += remainingMama;
        }
        mamaPerSecond = (mamaAmount * PRECISION) / duration;
        unchecked {
            latestPeriodNumber++;
            latestPeriodStartTime = currentTime + 1;
            latestPeriodEndTime = endTime;
            latestPeriodMamaPerSecond = mamaPerSecond;
        }
        emit NewUpkeepPeriod(latestPeriodNumber, currentTime + 1, endTime, mamaPerSecond, mamaAmount);
    }

    /// @notice Update mama reward for all the liquidity mining pool.
    ...
    /// @notice Update mama reward for the liquidity mining pool.
    ...
    /// @notice Safe Transfer MAMA.
    /// @param _to The MAMA receiver address.
    /// @param _amount Transfer MAMA amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            ...
            // Update mamaAmountBelongToMC
            unchecked {
                if (mamaAmountBelongToMC >= _amount) {
                    mamaAmountBelongToMC -= _amount;
                } else {
                    mamaAmountBelongToMC = balance - _amount;
                }
            }
            MAMA.safeTransfer(_to, _amount);
        }
    }
