...
async function main() {
  ...
  const MAMA = await SelfSufficientERC20.deploy();
  await MAMA.initlialize("MockDOGE", "DOGE", 18);
  console.log("MAMA", MAMA.address);
  await MAMA.mint(owner.address, BigNumber.from("10000000000000000000000000"));
  await MAMA.mint(
